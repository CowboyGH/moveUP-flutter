...
- Profile current phase section for the authenticated `/profile` tab, rendering the read-only phase block alongside the aggregate statistics frequency summary.
...
- The `/profile` screen now sources each section independently from focused `/api/profile/*` endpoints (user, phase, user-parameters, history, active-subscription) instead of a single bootstrap payload: the user card, current phase, parameters, subscription, and statistics/history sections each load and refresh themselves, and charts switch between dedicated volume/frequency/trend endpoints.
- Removed the cross-feature `ProfileRefreshCubit` workaround now that the subscription section reloads itself after purchase or cancellation.
